...
│   ├── 고객 여정 요약 카드 (완주율 + 대기 이탈률 + 호이관율 + 추세)
│   └── AI 인사이트 1줄 (규칙 기반)
│
...
| 권한 기반 노출 | 접근 권한 없는 서비스 카드 미노출 |
| 즐겨찾기 제한 | 즐겨찾기 슬롯 최대 5개 |
...
| H4 | 전체 서비스 (Bento Grid) | 접근 권한 있는 서비스만 카드 노출 / 사용 빈도 기반 카드 크기 차등 / Drag & Drop 순서 변경 |
| H4 | 전체 서비스 > 초기값 | 진입 빈도 1위 서비스가 없을 때(신규 사용자) 기본 Large 카드: 상담 어드바이저 포털 |
...
| HF | Funnel 요약 카드 | 운영 관리자 이상 노출 / 고객 여정 완주율(근사) + 대기 이탈률 + 호이관율 + 추세(▲▼) + AI 인사이트 1줄 / 상세는 9장 참조 |
...
| 규칙 | 상세 정책 |
|------|-----------|
| 카드 크기 차등 | 사용 빈도/AI 추천 기반으로 Large·Medium 크기 차등 부여 |
| Large 카드 | 가장 자주 쓰는(또는 AI 추천 1순위) 서비스 1개 / 핵심 요약 지표 함께 노출 |
| Medium 카드 | 그 외 서비스 / 아이콘·서비스명·설명·이상 징후 Badge 구성 |
...
| Funnel 요약 조회 | 고객 여정 단계별 집계 합산 결과(완주율·대기 이탈률·호이관율·추세) + AI 인사이트 1줄 반환 (내부 확보 데이터 기반, 외주는 연동만) |
...
## 9. AICC Journey 요약 (홈 Funnel 카드 + AI 인사이트)

> 별도 AICC Journey(Funnel) 화면은 두지 않는다. 고객 여정 분석은 **홈의 요약 카드 + AI 인사이트 1줄**로 축약 제공하며, 상세 분석 페이지·드릴다운·필터는 차기로 보류한다.

### 9.1 목적 및 범위

| 항목 | 내용 |
|------|------|
| 목적 | AICC 전체 서비스를 관통하는 고객 여정(S1~S6)을 홈에서 요약 인지 |
| 노출 대상 | 운영 관리자 등급 이상 (`AICC_FUNNEL:VIEW` 권한 보유자) / 현장 사용자 미노출 |
| 포함 | 홈 요약 카드 1장 + AI 인사이트 1줄 (읽기 전용) |
| 제외 (차기) | 별도 Funnel 페이지, 단계 드릴다운, 필터, 세그먼트 비교, 데이터 내보내기 |

### 9.2 데이터 정책 (단계별 집계 합산)

> 한 고객을 단계별로 잇는 **세션 단위 추적(여정 연결 키)은 사용하지 않는다.** 각 서비스가 제공하는 단계별 집계값을 합산하여 요약 지표를 산출한다.

| 항목 | 내용 |
|------|------|
| 집계 방식 | 단계별 집계 합산 (세션 단위 추적 / 여정 연결 키 미사용) |
| 단계 범위 | S1 진입 ~ S6 종료 전체 |
| 완주율 산출 | (해결 종료 건수 ÷ 전체 인입 건수) **근사치** — 세션 조인 없이 집계 비율로 계산 |
| 갱신 주기 | 일/시간 단위 배치 (실시간 아님) |
| 데이터 확보 | **내부(데이터/인프라팀)** 가 원천 확보·집계까지 책임 |
| 데이터 연동 | **외주**는 내부가 확보한 집계 데이터를 **API로 연동**하고 화면에 표시만 수행 |
...
| S4 | 연결 대기 | **대기 이탈률 ★** |
| S5 | 상담 진행 | 상담사 연결 규모 |
| S6 | 종료 | **호이관율 ★** / 해결 종료 → 완주율 |

### 9.4 요약 카드 UI

| No | 영역 | 상세 정책 |
|----|------|-----------|
| HF-1 | 헤드라인 지표 | End-to-End 완주율(근사) + 전주 대비 추세(▲▼) |
| HF-2 | 보조 지표 | 대기 이탈률 · 호이관율 (S4·S6 핵심 추적점) |
| HF-3 | AI 인사이트 1줄 | 규칙 기반 현상 요약 문구 (9.5) |
| HF-4 | 이상 징후 강조 | 이탈·호이관 급증 등 이상 감지 시 카드 강조 (8장 이상 징후 감지와 연동) |
| HF-5 | Empty/권한/표본부족 | 데이터 없음 / 권한 없음 / 표본 부족(분석 보류) 상태 처리 |
...
**In-Scope (외주)**
- 홈 Funnel 요약 카드 화면 구현 (완주율·대기 이탈률·호이관율·추세 + AI 인사이트 1줄)
- 내부 제공 Funnel 요약 API 연동
...
- 별도 Funnel 페이지·드릴다운·필터·세그먼트 비교
- LLM 기반 인사이트 자유 생성, 원인 후보 상관 분석, 예측 기능

---

## 부록 A. 인사이트 카탈로그 (현상 → 원인 후보 → Action)

> 기존 AICC Journey(Funnel) PRD 7장에서 이관. **원인은 "후보"이며 확정이 아니다.** 내부 데이터팀의 집계 설계 및 규칙 기반 인사이트 정의 시 참고한다. (출처 파일은 `U+AICC_Console_Funnel_PRD_(보관용).md` — 업데이트 중단)
...
| 이상 징후 | 특정 구간 지표 평소比 급변 | 단계별 복합 요인 | 홈 배너 알림 + 상세 분석 유도 | 전 단계 집계 (8장 연동) |

---

## 부록 B. 단계별 데이터 소스 매핑 (확보 현황)

> 기존 Funnel PRD 7.1에서 이관. 홈 Funnel 요약(9.2)은 **단계별 집계 합산** 방식이므로, 아래 소스의 단계별 집계값만 필요하며 세션 단위 조인 키는 사용하지 않는다.
...
| S4 연결 대기 | ACD/CTI 큐 대기·이탈 로그 | ★ ⚠️ 내부 확보 대상 |
| S5 상담 진행 (채널·상담유형) | 상담 어드바이저 코드유형·채널 데이터 | ★ ⚠️ 내부 확보 대상 |
| S6 종료 (호이관) | CTI 호 전환(transfer) 로그 | ★ ⚠️ 내부 확보 대상 |
| 재인입 (S6 이후) | `RP_REVISIT`, `RP_REVISIT_SUMMARY` | 콜봇 데이터 ✅ |

> S1~S3은 콜봇/챗봇 데이터로 집계 가능. S4~S6 집계 데이터는 **내부(데이터/인프라팀)** 확보가 선행되어야 하며, 확보 후 외주가 API로 연동한다. (세션 단위 여정 연결 키는 본 범위 제외)
